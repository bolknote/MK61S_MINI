...
3. приложение отправит firmware команду `uscreen` и выполнит handshake —
   выбирать режим на калькуляторе не требуется; терминальную CP1251 клиент
   декодирует локально и не посылает команд переключения кодировки;
...
прямые клавиши ПК, включая цифры и стрелки, бинарными событиями
`KEY_EVENT down/up`. Поэтому firmware видит удержание и сама решает, как
трактовать клавишу в текущем foreground. Только буквы и знаки без отдельной
клавиши матрицы передаются штатными последовательностями `kbd XX` через
terminal multiplex; повторные SMS-нажатия в этих последовательностях не
теряются.
...
с экраном. При открытии фокус переходит в строку команды; она принимает Unicode
с текущей системной раскладки, а перед отправкой строго кодирует его в CP1251.
Непредставимый символ не отправляется и показывает понятную ошибку.
