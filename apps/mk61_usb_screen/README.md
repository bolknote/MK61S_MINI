...
3. приложение само запросит `encoding utf-8`, отправит firmware команду
   `uscreen` и выполнит handshake — выбирать режим на калькуляторе не требуется;
   старый firmware или тесный F401/UC1609 профиль может отклонить выбор
   кодировки, и этот совместимый ответ приложение поглотит автоматически;
