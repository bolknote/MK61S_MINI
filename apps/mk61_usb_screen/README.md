...
2. оставить включённым автоподключение или выбрать CDC-порт вручную;
3. приложение само отправит firmware терминальную команду `uscreen` и выполнит
   handshake — выбирать режим на калькуляторе не требуется;
4. дождаться статуса «USB Screen подключён» — только тогда физический экран
   погаснет.

Автоподключение открывает только устройство с USB VID/PID STM32 CDC и product
identity платы MK61/BLACKPILL_F411CE. Одного имени `COM3`, `ttyUSB0` или
`usbmodem` недостаточно: приложение не пишет команды в неопознанные serial-порты.
Плата с изменёнными или недоступными USB descriptors остаётся в списке и может
быть подключена вручную — это явное разрешение пользователя на probe.

Автоматический вход работает из текущего foreground-интерфейса: калькулятора,
меню, редактора FOCAL/TinyBASIC и других экранов, обслуживающих общий idle-loop.
Пункт **Development → USB Screen** / **Разработка → USB-экран** остаётся ручным
...
режим. При штатном выходе firmware отправляет приложению `DETACH`: окно сразу
закрывает экранную сессию без ошибки, но оставляет CDC-порт открытым и не включает
USB Screen обратно вопреки пользователю. Для новой сессии нажмите появившуюся
кнопку «Включить USB Screen» либо снова выберите режим на самом устройстве.
Физическое переподключение USB считается новым подключением и снова запускает
автоматический вход.

При закрытии приложение передаёт `RELEASE_ALL_KEYS`/`DETACH`, вызывает serial
`drain()` и лишь затем закрывает дескриптор. Поэтому штатный выход не полагается
на случайную доставку последних байтов из очереди драйвера.
...
- `+`, `-`, `*`, `/`, `^`, точка, пробел и поддерживаемые редакторами знаки
  `! @ # $ % & ( ) : ; , " = ' < >`;
- стрелки `←↑↓→`, Backspace/Delete, Enter → `OK`, Escape → `ESC`; на Mini
  `↑` передаётся как `←ШГ`, а `↓` — как `ШГ→`;
...
Стрелки поглощаются до стандартных Flutter shortcuts: они не перемещают фокус и
не прокручивают интерфейс приложения после отправки в MK61.
