...
Каждая игра находится в отдельной папке: `autoexec.m61` или `autoexec.tbi`,
`manual.md` и, где они нужны, иллюстрации `.wbmp`. Скопируйте нужную папку
целиком в `/games` на USB-диске MK61S. Вход в папку запускает `autoexec`.
Исторические M61-игры сначала открывают памятку; High Noon запускается
M61-диспетчером из четырёх частей TinyBASIC и показывает полную инструкцию
сама.
...
изображения выше экрана доступны с прокруткой. M61 и памятки помещаются в
1536 байт, части TinyBASIC — в 3584 байта, каждое изображение — в 1600 байт.

## Каталог

| Папка | Игра и памятка | Иллюстрация |
| --- | --- | --- |
| `ELITE` | [ELITE](ELITE/manual.md) — 256 миров, торговля, пираты и tHArGOId | Сегментный индикатор, 12 позиций; нужны расширения 1F/2F и 55/56 |
...
| `High Noon` | [High Noon](<High Noon/manual.md>) — дуэль с Чёрным Бартом | Текстовый экран 40×10, узкий внешний шрифт |
...
К прежнему содержимому каждой исторической `.m61` добавлена только первая строка
