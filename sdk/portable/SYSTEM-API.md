...
BASIC, WBMP, Markdown, CHIP-8, SETUP и USBDISK — обычные перемещаемые APP ABI 6. Они
...
вытесняется.
Долгоживущий MSC-сеанс дополнительно закрепляет `USBDISK.APP`: до парного `unpin`
загрузчик не позволяет заменить его другим образом.
...
Оба экспорта объявлены слабым контрактом для совместимости с прямой сборкой
Generic STM32F4 в Arduino IDE. Если стандартный linker script их не создаёт,
resident берёт нижнюю границу из `align8(_end)`, а верхнюю — из того же
профиля защиты стека F401/F411. Отдельный RAM-массив при этом не появляется.
