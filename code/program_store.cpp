...
// байтов хвостового заполнения, которые массив {u16, Inode} расходовал бы на
// каждый слот наложения.
...
// в одно слово. Это увеличивает активный набор промежуточных данных вчетверо,
// добавляя менее одного КиБ по сравнению с прежними параллельными массивами
// ключей, поколений и ссылок.
...
// хотя физическая микросхема и её сектор настроек останутся прежними. Тогда
// старый каталог невозможно подключить, но повторная разрушающая проверка
// ёмкости и стирание настроек были бы излишними и неожиданными. Этот узкий
// декодер доверяет только полностью зафиксированному указателю с CRC и отдельно
// защищённому CRC маркеру на неизменившемся физическом конце.
...
static bool select_reclaimable_sector(u32& out) {
  ...
  // За один проход таблицы inode отмечаем актуальность 32 кандидатов. Прежний
  // цикл по секторам мог сканировать всю таблицу узлов отдельно для каждого
  // сектора данных при почти полном носителе (квадратичный худший случай).
...
static bool format_internal(bool erase_settings) {
  ...
  // checkpoint() сама стирает и публикует новый активный банк. Второй банк
  // привязан к эпохе и будет стёрт, когда в следующий раз станет назначением
  // COW, поэтому немедленное стирание обоих банков здесь лишь утраивало работу
  // с каталогом при первой загрузке W25Q128 ёмкостью 16 МиБ.
  if(!checkpoint()) return false;
  // Заголовки промежуточных данных также содержат эпоху формата. После этого
  // форматирования старые или посторонние секторы игнорируются и стираются
  // отложенно перед первым добавлением в них.
  if(erase_settings) {
...
void init(void) {
  ...
  if(!load_locator()) {
    ...
    // Действительный указатель доказывает, что это существующий том C5. Потеря
...
bool remove_tree(u16 id, u16* removed) {
  ...
  // Обратный обход для удаления без стека. MAX_DIRECTORY_DEPTH ограничивает
  // проверки повреждений, а переход по first_child после каждого зафиксированного
  // удаления гарантирует, что при пропадании питания останется лишь меньшее,
  // но всё ещё согласованное поддерево.
  u16 current = id;
  u16 count = 0;
  while(true) {
    Inode inode;
...
// Ниже реализован постоянный журнал промежуточных данных USB.
...
static bool append_stage_value(u16 sector, u32 key, const u8* data) {
  ...
  record[0] = 'S';
  record[1] = '5';
  // Полная запись программируется единым проверяемым потоком. Восстановление
  // принимает только записи ACTIVE с совпадающей CRC данных, поэтому пропадание
  // питания во время программирования любой составляющей страницы NOR не может
  // заменить предыдущую версию.
...
// Завершает любое прерванное уплотнение. Перед стиранием старого сектора копии
// всегда получают более новое поколение, поэтому при каждом пропадании питания
// остаётся хотя бы одна действительная версия каждого промежуточного блока.
static bool recover_stage_reserve(void) {
  ...
  // Обычный сектор назначения уже может содержать первые записи, скопированные
  // обратно до сброса. Завершаем это направление, если его стёртого хвоста
  // достаточно.
  ...
  // Иначе сброс произошёл при копировании разреженного сектора-жертвы в резерв.
  // Завершаем копирование, стираем жертву и копируем данные обратно.
  ...
  for(u16 sector = 0; sector < g_geometry.stage_sector_count; sector++) {
    ...
    for(u8 slot = 0; slot < STAGE_RECORDS_PER_SECTOR; slot++) {
      ...
      if(!read_bytes(stage_record_address(ref) + STAGE_RECORD_HEADER_SIZE,
        ...
        // Никогда не добавляем данные после оборванной записи: сохранение
        // предыдущего действительного поколения важнее неиспользованного хвоста
        // этого сектора.
        g_stage_sealed[sector] = 1;
        continue;
      }
