...
// Логический размер файла, доступный через хранилище и виртуальную FAT.
...
// Согласовано с обходом виртуальной FAT. Это ограничивает обход предков без
// стека и не допускает дерево, которое невозможно представить через USB.
...
// Импорт по USB избегает задержки сжатия, пока несжатые данные помещаются
// в хранилище.
