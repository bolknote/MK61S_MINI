...
static bool select_angle_unit(i32 keycode, bool draw_labels) {
  AngleUnit angle;
  switch(keycode) {
    case KEY_DEGREE: angle = DEGREE; break;
    case KEY_GRADE:  angle = GRADE;  break;
    case KEY_RADIAN: angle = RADIAN; break;
    default: return false;
  }

  MK61Emu_SetAngleUnit(angle);
  if(draw_labels) GRDLabel.print(angle);
  angle_save.schedule(millis(), ANGLE_SAVE_UPDATE_MS);
  return true;
}
...
void key_press_handler(i32 keycode) {
  ...
  #endif
  if(select_angle_unit(keycode, true)) return;
...
static void service_m61_controls(void) {
  ...
  // Управляющие клавиши M61 фиксируются уже по первому прочитанному фронту
  // матрицы. Штатное debounced-событие остаётся в общей очереди, но короткое
  // нажатие больше не исчезает, если отпущено раньше 30 мс.
...
void  loop() {
  ...
  if(m61_text::active()) {
    if(core_61::is_RUN() || m61_text::calculator_suspended()) {
      kbd::scan_m61_controls();
    } else {
      kbd::scan();
    }
  }
...
static bool v2_poweroff_lse_context(void) {
  return input_focus == &mk61_baseloop_hook &&
         core_61::is_CALC() &&
         !core_61::edit_program &&
         !lcd_hooked &&
         !m61_text::active() &&
...
static void v2_show_poweroff_lines(const char* ru0, const char* ru1,
                                   const char* en0, const char* en1) {
  {
    MK61DisplayUpdate update(main_lcd());
    main_lcd().clear();
    main_lcd().cursorOff();
...
static void v2_release_lcd_db7_for_lse(void) {
  // Последняя запись в HD44780 должна завершиться до того, как PC15 перестанет
  // быть DB7. RW=0 оставляет вывод LCD входом, E=0 запрещает ему принимать
  // такты, пока на общей линии работает OSC32_OUT.
...
static void v2_restore_lcd_db7(void) {
  pinMode(PIN_LCD_DB7, OUTPUT);
  digitalWrite(PIN_LCD_DB7, LOW);
  pinMode(PIN_LCD_RW, OUTPUT);
  digitalWrite(PIN_LCD_RW, LOW);
...
static void v2_wait_without_lcd(void) {
  // При успешной передаче PC15 дисплею больше нельзя обращаться до сброса:
  // следующий запуск первым делом остановит сохранённый LSE и вернёт DB7.
  while(true) __WFI();
}

static void v2_prepare_rtc_for_poweroff(void) {
  kbd::clear_hold_key();
  library_mk61::flush_settings_state();
  sound_stop();

  v2_show_poweroff_lines(
    "ПОСЛЕ ЗВУКА", "МОЖНО ВЫКЛЮЧИТЬ",
    "AFTER THE BEEP", "THEN POWER OFF");
  v2_release_lcd_db7_for_lse();

  const rtc_clock::PoweroffLseResult result =
      rtc_clock::switch_to_lse_for_poweroff();
  if(result == rtc_clock::PoweroffLseResult::READY) {
    // Звук является подтверждением: только к этому моменту LSERDY установлен,
    // RTC действительно переведён на LSE и календарь проверен.
    led::on();
    sound(PIN_BUZZER, 2500, 300, 10);
    delay_with_sound_poll(350);
    sound_stop();
    v2_wait_without_lcd();
  }

  if(result == rtc_clock::PoweroffLseResult::GPIO_RELEASE_FAILED) {
    // PC15 всё ещё может принадлежать генератору. Не включаем DB7 на выход и
    // не подаём подтверждающий звук: надпись прямо требует дождаться звука.
    led::off();
    v2_wait_without_lcd();
  }

  v2_restore_lcd_db7();
  if(result == rtc_clock::PoweroffLseResult::TIME_NOT_SET) {
    v2_show_poweroff_lines(
...
void event_hold_key(i32 holded_key, i32 hold_quant) {
  switch(holded_key) {
      #if MK61_V2_RTC_POWEROFF_HANDOFF
      case KEY_CX:
          if(hold_quant == 0 && v2_poweroff_lse_context()) {
            dbgln(MENU, "HOLD [Cx]: prepare RTC LSE for poweroff");
            v2_prepare_rtc_for_poweroff();
          }
          kbd::clear_hold_key();
        break;
      #endif
