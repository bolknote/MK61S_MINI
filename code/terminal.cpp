...
static bool terminal_vfat_log(const char* args) {
  if(!terminal_core::at_end(args)) {
    char token[6];
    if(!terminal_single_token(args, token, sizeof(token)) ||
       strcmp(token, "clear") != 0) {
      Serial.println("Usage: vlog [clear]");
      return false;
    }
...
m61_ansi::SavedCursor class_terminal::print_saved_cursor = {};
u8 class_terminal::input_buffer[MAX_INPUT_CHAR] = {};
char  class_terminal::hist_text[HISTORY_TEXT_SIZE] = {};
u16   class_terminal::hist_start[HISTORY_DEPTH] = {};
u8    class_terminal::hist_length[HISTORY_DEPTH] = {};
u8    class_terminal::hist_head = {};
u8    class_terminal::hist_count = {};
u16   class_terminal::hist_used = {};
u16   class_terminal::hist_write = {};
i8    class_terminal::hist_nav = {};
terminal_line_editor::EscapeDecoder class_terminal::escape_decoder = {};
u8    class_terminal::prev_terminator = {};
u8    class_terminal::saved_line[MAX_INPUT_CHAR] = {};
usize class_terminal::saved_len = {};
usize class_terminal::saved_cursor = {};

void class_terminal::history_drop_oldest(void) {
      hist_used -= hist_length[hist_head];
      hist_head = (u8) ((hist_head + 1) % HISTORY_DEPTH);
      hist_count--;
    }

void class_terminal::history_entry_read(u8 slot, u8* out) {
      const u16 start = hist_start[slot];
      for(u8 i = 0; i < hist_length[slot]; i++) out[i] = (u8) hist_text[(u16) ((start + i) % HISTORY_TEXT_SIZE)];
    }

bool class_terminal::history_entry_equals(u8 slot, const u8* line, usize len) {
...
void class_terminal::history_add(const u8* line, usize len) {
...
void class_terminal::history_print(void) {
      if(hist_count == 0) {
...
void class_terminal::print_prompt(void) {
      char path[64];
...
void class_terminal::move_terminal_cursor(char direction, usize columns) {
      if(columns == 0) return;
      Serial.write(0x1B);
      Serial.write('[');
...
void class_terminal::history_recall(i8 nav) {
...
void class_terminal::history_key_up(void) {
...
void class_terminal::history_key_down(void) {
      if(hist_nav < 0) return;
      history_recall((i8) (hist_nav - 1));
    }

void class_terminal::editor_move_left(void) {
...
void class_terminal::editor_move_right(void) {
...
void class_terminal::editor_delete_forward(void) {
...
bool class_terminal::editor_insert(u8 byte) {
...
void class_terminal::editor_key(terminal_line_editor::Key key) {
      switch(key) {
...
void class_terminal::print_crash_usage(void) {
...
terminal_protocol::Result class_terminal::show_crash_dump(void) {
...
terminal_protocol::Result class_terminal::save_crash_dump(const char* path) {
      crash_dump_format::Record record = {};
      if(!crash_dump::copy(record)) {
...
terminal_protocol::Result class_terminal::exec_crash(void) {
...
void class_terminal::print_mpu_usage(void) {
...
terminal_protocol::Result class_terminal::show_mpu_status(void) {
...
terminal_protocol::Result class_terminal::exec_mpu(void) {
      const char* cursor = command_args();
...
const char* class_terminal::watchdog_state_name(u32 state) {
      switch(state) {
...
void class_terminal::print_watchdog_usage(void) {
...
terminal_protocol::Result class_terminal::show_watchdog_status(void) {
...
terminal_protocol::Result class_terminal::exec_watchdog(void) {
      const char* cursor = command_args();
...
#if defined(MK61_OLED1602_WS0010)
void class_terminal::display_test_text(void) {
...
void class_terminal::display_test_map(u8 page) {
      main_lcd().endWs0010Graphics();
      main_lcd().clear();
...
void class_terminal::display_test_alphabet(u8 page) {
      main_lcd().endWs0010Graphics();
      lcd_ru::restore_default_font();
      main_lcd().clear();
      if(page < 2) {
...
void class_terminal::display_test_symbols(void) {
      main_lcd().endWs0010Graphics();
      lcd_ru::restore_default_font();
      main_lcd().clear();
...
void class_terminal::make_display_ddram(u8 cells[lcd_display::ROWS]
...
void class_terminal::display_test_cgram(void) {
      main_lcd().endWs0010Graphics();
      main_lcd().clear();
...
terminal_protocol::Result class_terminal::display_test_graphics(u8 pattern) {
...
#if MK61_WS0010_GRAPHICS_PROFILE_QUALIFIED && \
    MK61_WS0010_GRAPHICS_100X16
terminal_protocol::Result class_terminal::display_test_graphics_readback(
        u8 pattern) {
...
#endif
#endif
terminal_protocol::Result class_terminal::exec_display(void) {
      const char* cursor = command_args();
      if(terminal_core::at_end(cursor)) {
...
const char* class_terminal::power_state_name(
...
void class_terminal::print_power_status(void) {
...
void class_terminal::print_analog_status(void) {
...
#endif
void class_terminal::print_u64(u64 value) {
      char digits[21];
      usize length = 0;
      do {
        digits[length++] = (char) ('0' + value % 10U);
...
void class_terminal::print_profile_statistics(void) {
      terminal_output::field(
      ...
      terminal_output::line(Serial, "KBD overflow=", kbd::overflow_count());
...
#if MK61_ENABLE_PROFILE_SAVE
u16 class_terminal::build_profile_report(u8* output, usize capacity) {
...
terminal_protocol::Result class_terminal::save_profile_report(const char* path) {
      storage_path::FileTarget target = {};
      const storage_path::Status path_status = storage_path::file_target(
          current_directory, path, program_store::ProgramType::TEXT, target);
      if(path_status != storage_path::Status::OK) {
...
#endif
terminal_protocol::Result class_terminal::exec_profile(void) {
...
#endif
terminal_protocol::Result class_terminal::script_action(terminal_protocol::ResultKind action, const char* args) {
      recive_pos = 0;
      return terminal_protocol::Result::action(action, args);
    }

bool class_terminal::input_can_append(void) const {
      return terminal_core::input_can_append(recive_pos);
    }

void class_terminal::list_mk61_code_page(void) {
...
void class_terminal::dump_mk61_code_page(void) {
      u8 code_page[core_61::CODE_PAGE_BUFFER_SIZE] = {};
      core_61::get_code_page(&code_page[0]);
...
terminal_protocol::Result class_terminal::exec_identity(void) {
...
#if MK61_ENABLE_READ_BENCHMARKS
void class_terminal::print_benchmark_pass(u8 pass, u32 elapsed_us, u32 crc) {
...
terminal_protocol::Result class_terminal::print_benchmark_summary(
...
terminal_protocol::Result class_terminal::benchmark_file(const char* cursor) {
      char path[MAX_INPUT_CHAR];
      usize passes = 5;
...
terminal_protocol::Result class_terminal::benchmark_flash(const char* cursor) {
...
terminal_protocol::Result class_terminal::benchmark_random(const char* cursor) {
      usize reads = 1024;
      usize passes = 3;
      if(!terminal_core::at_end(cursor) &&
...
terminal_protocol::Result class_terminal::exec_benchmark(void) {
      const char* cursor = command_args();
      char kind[8];
...
#endif
terminal_protocol::Result class_terminal::exec_date(void) {
      const char* args = command_args();
...
char class_terminal::alarm_id_letter(rtc_clock::AlarmId id) {
      return id == rtc_clock::AlarmId::A ? 'A' : 'B';
    }

void class_terminal::print_two_digits(u8 value) {
      Serial.write((u8) ('0' + value / 10));
      Serial.write((u8) ('0' + value % 10));
    }

void class_terminal::print_alarm_line(rtc_clock::AlarmId id) {
...
void class_terminal::print_alarm_status(void) {
...
terminal_protocol::Result class_terminal::exec_alarm(void) {
      const char* cursor = command_args();
      if(terminal_core::at_end(cursor)) {
        print_alarm_status();
        return terminal_protocol::Result::ok();
...
void class_terminal::cancel_file_upload(void) {
...
bool class_terminal::read_staged_file_upload(void* context, u32 offset,
      ...
      while(size != 0) {
        ...
        const u16 in_block =
            (u16) (offset % program_store::VFAT_STAGE_BLOCK_SIZE);
        usize count = program_store::VFAT_STAGE_BLOCK_SIZE - in_block;
        if(count > size) count = size;
        if(block_index >= FILE_UPLOAD_STAGE_BLOCKS ||
           !program_store::vfat_stage_read(
               FILE_UPLOAD_STAGE_FIRST_KEY + block_index, block)) {
          return false;
        }
        memcpy(output, block + in_block, count);
        output += count;
        offset += (u32) count;
        size -= count;
      }
...
terminal_protocol::Result class_terminal::file_transfer_error(const char* code) {
      cancel_file_upload();
...
terminal_protocol::Result class_terminal::exec_fs_get(void) {
      char path[MAX_INPUT_CHAR];
...
terminal_protocol::Result class_terminal::exec_fs_put(void) {
      const char* cursor = command_args();
      char action[16];
      if(!terminal_core::parse_token(cursor, action, sizeof(action))) {
      ...
      if(strcmp(action, "data") == 0) {
        ...
        usize copied = 0;
        while(copied < decoded_length) {
          const u16 position = (u16) (offset + copied);
          const u16 block_index =
              position / program_store::VFAT_STAGE_BLOCK_SIZE;
          const u16 in_block =
              position % program_store::VFAT_STAGE_BLOCK_SIZE;
          usize count = program_store::VFAT_STAGE_BLOCK_SIZE - in_block;
          if(count > decoded_length - copied) {
            count = decoded_length - copied;
          }
          memcpy((u8*) workspace.data() + in_block,
                 decoded + copied, count);
          for(usize index = 0; index < count; index++) {
            file_upload_checksum_state =
                terminal_file_transfer::checksum_update(
                    file_upload_checksum_state, decoded[copied + index]);
          }
          copied += count;
          if(in_block + count == program_store::VFAT_STAGE_BLOCK_SIZE) {
            if(block_index >= FILE_UPLOAD_STAGE_BLOCKS ||
               !program_store::vfat_stage_write(
                   FILE_UPLOAD_STAGE_FIRST_KEY + block_index,
                   (const u8*) workspace.data())) {
              return file_transfer_error("PUT_IO");
            }
            memset(workspace.data(), 0,
      ...
      if(strcmp(action, "end") == 0) {
        ...
        if(file_upload_size % program_store::VFAT_STAGE_BLOCK_SIZE != 0) {
          const u16 block =
              file_upload_size / program_store::VFAT_STAGE_BLOCK_SIZE;
          if(block >= FILE_UPLOAD_STAGE_BLOCKS ||
             !program_store::vfat_stage_write(
                 FILE_UPLOAD_STAGE_FIRST_KEY + block,
                 (const u8*) workspace.data())) {
            return file_transfer_error("PUT_IO");
          }
        }
        const u32 crc = terminal_file_transfer::checksum_finish(
...
#if MK61_ANY_LOADABLE_MODULE
        if(file_upload_target.type == program_store::ProgramType::APP) {
          const loadable_module::ModuleSource app_source = {
            &file_upload_size, file_upload_size, read_staged_file_upload
          };
          loadable_module::Header header = {};
          const loadable_module::StoreStatus status =
              loadable_module::validate_app(app_source, header);
          if(status ==
             loadable_module::StoreStatus::INCOMPATIBLE_FIRMWARE) {
            return file_transfer_error("PUT_FIRMWARE");
          }
          if(status != loadable_module::StoreStatus::OK) {
            return file_transfer_error("PUT_APP");
          }
        }
#endif
        if(!program_store::write_file_from_source(
             file_upload_target.parent_id, program_store::INVALID_ID,
             file_upload_target.type, file_upload_target.name,
             file_upload_size, source)) {
          return file_transfer_error("PUT_IO");
        }
...
void class_terminal::DumpRegisters(void) {
     // -0.12345678 -9A (14)
      char buffer[15];
...
void class_terminal::Dump1302(void) {
      char buff[43];
      MK61Emu_get_1302_R(&buff[0]);
      Serial.println(buff);
    }

void  class_terminal::print_address_as_MK61(usize addr) {
      if(addr > 99) {
        Serial.write('A');
...
void class_terminal::reset_command_state(void) {
...
void class_terminal::reset_line_editor(void) {
...
void class_terminal::init_script(void) {
      current_directory = program_store::ROOT_ID;
      reset_command_state();
    }

void  class_terminal::echo_mk61_stack(void) {
      char cvalue[15];
      cvalue[14] = 0;
...
void  class_terminal::echo_ISA_61(void) {
...
terminal_protocol::Result class_terminal::command_to_kbd(bool script_mode) {
      ...
      u32 sequence = terminal_keys::sequence((u8) code_61);
      ...
      for(isize i=0; i<4; i++) {
        ...
        // Скрипт адресует ядро, а не текущий foreground (это может быть меню).
...
bool class_terminal::scancode_to_kbd(i32& out) {
      usize key_code = 0;
...
bool class_terminal::value_fits_mk61(double value) {
      char sign = ' ';
      char mantissa[8];
      isize pow10 = 0;
...
const char* class_terminal::command_args(void) {
...
isize class_terminal::parse_u32_list(const char* p, u32* out, usize max_count) {
      usize count = 0;
      while(true) {
        while(*p == ' ' || *p == ',') p++;
        if(*p == 0) break;
...
bool class_terminal::exec_led(void) {
...
bool class_terminal::exec_beep(void) {
...
bool class_terminal::parse_mk_display_value(const char* v, double& out) {
      char buffer[20];
      char* w = buffer;
      if(v[0] == '-') *w++ = '-';
      for(int i = 1; i <= 9; i++) {
...
bool class_terminal::operand_delimiter(char c) {
      return terminal_core::is_end(c) || terminal_core::is_space(c) || c == '<' || c == '>' || c == '=' || c == '!';
    }

bool class_terminal::parse_if_operand(const char*& p, double& out) {
...
bool class_terminal::print_value(const m61_print::ValueRef& value, void* user_data) {
      PrintRenderContext& context = *(PrintRenderContext*) user_data;
      char text[15];
      text[14] = 0;
...
terminal_protocol::Result class_terminal::exec_if(bool script_mode, bool trap_mode) {
...
terminal_protocol::Result class_terminal::execute(bool script_mode,
        ...
        const u8 command_id = terminal_catalog::lookup(&input_buffer[0]);
        ...
        switch (command_id) {
          ...
          case CMD_FS_LIST: {
              ...
              Serial.print(stored_count);
              Serial.println(stored_count == 1 ? " entry." : " entries.");
            }
            break;
          case CMD_FS_MKDIR: {
              const char* cursor = command_args();
...
class_terminal::InputResult class_terminal::input_handler(u8 rx_char) {
...
i32 class_terminal::serial_input_handler() {
...
terminal_protocol::Result class_terminal::execute_script_line(
