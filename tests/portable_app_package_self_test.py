...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="mk61-package-") as directory:
        work = Path(directory)
        image, app, memory, relocations = (
            work / x for x in ("image.bin", "test.APP", "memory.bin", "empty.rel"))
        relocations.write_bytes(b"")
        calls = bytearray()
        ...
        for original, choice in fixtures:
            ...
            assert struct.unpack_from("<II", packed, 40) == (
                min(plain, bcj), 0)  # Compressed code and empty relocations.
