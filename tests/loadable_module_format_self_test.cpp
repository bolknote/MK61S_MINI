...
static void test_kind_file_names(void) {
  ...
  assert(kind_at(4) == Kind::MARKDOWN_VIEWER);
  ...
  assert(strcmp(file_name(Kind::MARKDOWN_VIEWER), "MARKDOWN.APP") == 0);
  ...
  assert(kind_from_file_name("chip8.app", kind) &&
         kind == Kind::CHIP8);
  assert(kind_from_file_name("Markdown.App", kind) &&
         kind == Kind::MARKDOWN_VIEWER);
  assert(kind_from_file_name("usbdisk.app", kind) &&
         kind == Kind::USBDISK);
...
static void test_uncompressed_payload(void) {
  ...
  MemoryReader memory = {source, sizeof(source), 0, 0};
...
static void test_buffered_crc_across_blocks(void) {
  ...
  // 1025 байт пересекают две полные внутренние порции по 512 байт и последнюю
  // неполную. Ограничение reader дополнительно фиксирует размер порции.
  MemoryReader memory = {source, sizeof(source), 512, 0};
  const Reader reader = {&memory, read_memory};
  u8 output[sizeof(source)] = {};
  DecodeResult result = {};
  assert(decode_payload(reader, Compression::NONE, sizeof(source), output,
...
static void test_zx0_payload(void) {
  ...
  MemoryReader memory = {packed, sizeof(packed), 0, 0};
  const Reader reader = {&memory, read_memory};
  ...
  MemoryReader run_memory = {run, sizeof(run), 0, 0};
  ...
  MemoryReader short_memory = {
    short_match, sizeof(short_match), 0, 0
  };
...
static void test_zx0_rejects_bad_streams(void) {
  ...
  MemoryReader memory = {truncated, sizeof(truncated), 0, 0};
  const Reader reader = {&memory, read_memory};
  u8 output[10] = {};
  DecodeResult result = {};
  ...
  MemoryReader trailing_memory = {trailing, sizeof(trailing), 0, 0};
  ...
  MemoryReader valid_memory = {valid, sizeof(valid), 0, 0};
...
static void test_zx0_corruption_is_bounded(void) {
  ...
  for(u32 size = 1; size < sizeof(packed); size++) {
    MemoryReader memory = {packed, size, 0, 0};
    const Reader reader = {&memory, read_memory};
  ...
  for(usize byte = 0; byte < sizeof(packed); byte++) {
    for(u8 bit = 0; bit < 8; bit++) {
      ...
      MemoryReader memory = {damaged, sizeof(damaged), 0, 0};
      const Reader reader = {&memory, read_memory};
  ...
  for(usize sample = 0; sample < 256; sample++) {
    ...
    MemoryReader memory = {noise, size, 0, 0};
    const Reader reader = {&memory, read_memory};
    (void) decode_payload(reader, Compression::ZX0, size, output,
                          sizeof(output), result);
  }
