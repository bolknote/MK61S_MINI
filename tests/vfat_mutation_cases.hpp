...
static void test_bounded_vfat_mutations(void) {
  ...
  for(usize trial = 0; trial < cases; ++trial) {
    ...
    // Verify persistent C6 as well, not merely a cached logical read.
    virtual_fat::end_session();
    program_store::init();
    if(!virtual_fat::reset_session()) {
      char report[virtual_fat::DIAGNOSTIC_LINE_SIZE];
      virtual_fat::format_diagnostic(virtual_fat::diagnostic(), report,
                                     sizeof(report));
      std::fprintf(stderr, "VFAT recovery failed: trial=%u %s stage=%u\n",
                   (unsigned) trial, report,
                   (unsigned) program_store::vfat_stage_count());
      assert(false);
    }
    expect_file(60, keep, sizeof(keep)-1);
  }
  std::printf("VFAT mutation corpus: cases=%u rejected=%u C6 preserved PASS\n",
