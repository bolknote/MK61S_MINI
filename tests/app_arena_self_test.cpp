...
int main() {
  static_assert(APP_MAX_SIZE == 20480);
  const usize total = capacity(Arena::OVERLAY);
  assert(total > APP_MAX_SIZE && capacity(Arena::APP) == 0);
  for(usize bytes : {usize(1), usize(1924), usize(3356), usize(9152), usize(17252), usize(20140), usize(20480)}) {
    ...
    assert(app.size() == bytes && (uintptr_t(app.data()) & 31) == 0);
    assert(app.set_evictable(evict));
    const usize free = total - ((bytes + 31) & ~usize(31));
    assert(capacity(Arena::OVERLAY) == free);
    assert(capacity(Arena::APP) + free == total);
    ...
    pinned = false;
    assert(prefix.acquire_cache(Arena::OVERLAY, Owner::VFAT_STAGE, total));
