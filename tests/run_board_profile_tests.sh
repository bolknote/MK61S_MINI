...
clang++ "${common[@]}" -DREVISION_V3 -DSTM32F411xE \
  -DMK61_OLED1602_WS0010 -DMK61_CONFIG_EXPECT_V3 \
  -DMK61_CONFIG_EXPECT_WS0010 \
  -DMK61_CONFIG_EXPECT_WS0010_GRAPHICS_QUALIFIED \
  -DMK61_CONFIG_EXPECT_FULLSCREEN_BITMAP -o "$out-f411-ws0010-qualified"
"$out-f411-ws0010-qualified"

clang++ "${common[@]}" -DREVISION_V3 -DSTM32F401xC \
  -DMK61_OLED1602_WS0010 -DMK61_CONFIG_EXPECT_V3 \
  -DMK61_CONFIG_EXPECT_WS0010 -o "$out-f401-ws0010-unqualified"
"$out-f401-ws0010-unqualified"
...
if clang++ "${common[@]}" -DREVISION_V3 -DMK61_CONFIG_EXPECT_V3 \
    ...
    >/dev/null 2>&1; then
  echo "obsolete separate user APP flag unexpectedly compiled" >&2
  exit 1
fi

if clang++ "${common[@]}" -DREVISION_V3 -DMK61_CONFIG_EXPECT_V3 \
    -DMK61_ENABLE_PORTABLE_APPS=1 -o "$out-obsolete-portable-apps" \
    >/dev/null 2>&1; then
  echo "obsolete portable APP selector unexpectedly compiled" >&2
  exit 1
fi

if clang++ "${common[@]}" -DREVISION_V3 -DMK61_CONFIG_EXPECT_V3 \
    -DMK61_EXTERNALIZE_SYSTEM_APPS=1 -o "$out-obsolete-system-apps" \
    >/dev/null 2>&1; then
  echo "obsolete System APP externalization flag unexpectedly compiled" >&2
  exit 1
fi
