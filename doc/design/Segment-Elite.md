...
23.09.2026. Второй этап оптимизации, прошивка `544269E1`. Документ описывает собранную
...
открывает `part00.m61`…`part05.m61` и запускает игру. В каждой части до шести
банков: это укладывается в лимит 1536 байт на файл. Для загрузки нужна
прошивка, сохраняющая банки при вложенном `open`; старое поведение очищало
предыдущие части. Проверка исполняет настоящий M61-runner вместе с ядром.
...
| B02 | 0224–0335 | display / station |
| B03 | 0336–0447 | glyphs / messages |
| B04 | 0448–0559 | arithmetic |
| B05 | 0560–0671 | world |
| B06 | 0672–0783 | prices |
| B07 | 0784–0895 | trade |
| B08 | 0896–1007 | station |
| B09 | 1008–1119 | navigation / combat views |
...
| B19 | 2128–2239 | format / trade |
| B20 | 2240–2351 | views |
| B21 | 2352–2463 | views |
| B22 | 2464–2575 | flight |
| B23 | 2576–2687 | combat |
| B24 | 2688–2799 | data / kernel |
| B25 | 2800–2911 | data / contacts |
| B26 | 2912–3023 | data / weapons |
| B27 | 3024–3135 | data |
| B28 | 3136–3247 | data |
| B29 | 3248–3359 | data |
| B30 | 3360–3471 | combat |
| B31 | 3472–3583 | enemies |
...
В бою между командами рабочий R6 хранит число живых дронов, R8 — броню
выбранной цели. Приборы и отказанные команды сохраняют эти значения.
Оба кеша заполняются при встрече и обновляются после каждого хода; выбор
живой цели обновляет R8. Внутри хода R8 временно несёт флаг выпуска дрона.
...
| B29 FRAME | четыре слова кадра, четыре свободных поля, постоянное слово СП |
...
заряд сразу, без повторных чтений страницы. После хода отображается броня
цели (`H`), в том числе ноль после уничтожения; расстояние доступно по `9`.
...
внутри ядра. Повторная копия в R4…R7 не нужна; эти четыре поля свободны.
...
В таблице масок оставлены только используемые номера товаров 0…6;
