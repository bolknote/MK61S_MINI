...
В расширенном режиме памяти также доступен регистр `.RF`. В обычном режиме
`.RF` не используется и считается ошибочной ссылкой.

Ссылки на стек и регистры можно использовать в выражениях, в `LET` и в `IN`:
...
Ссылки МК-61 не используются как переменные цикла. В `FOR` должна стоять
обычная переменная БЕЙСИКа, например:

```text
FOR A=1 TO 10:.R0=A:NXT A
```
...
Регистр `RF` относится к расширенному режиму памяти.
