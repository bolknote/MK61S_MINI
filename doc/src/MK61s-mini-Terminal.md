...
`TXT`, `STATE.TXT`, `FMK`, `WBMP` и `APP`. Программы, тексты и шрифты
ограничены 1536 байтами, изображения `.wbmp` — 1600 байтами, исполняемые
контейнеры `.app` — 20 544 байтами. Basename занимает до 31 байта корректного
...
большой APP и расширение большого каталога дополнительно расходуют служебные
узлы. Точные значения установленного устройства показывает `df`.
...
Системные компоненты имеют тот же формат. FOCAL, TinyBASIC и просмотрщик WBMP
ищутся под каноническими путями `/System/FOCAL.APP`, `/System/BASIC.APP` и
`/System/WBMP.APP`. `System` — обычный видимый каталог: его можно создать и
...
Проводник. Пользовательский `.app` загружается в общий 20-КиБ SRAM-overlay и
...
`.m61`, `.foc`, `.tbi`, `.txt`, `.state.txt`, `.fmk`, `.wbmp`, `.app` и старые
псевдонимы `.t1`, `.m2`, `.wbm`; действуют обычные ограничения C5 по имени и
размеру. APP размером от 64 до 20 544 байт можно передавать по любому пути.
