...
### Загружаемые модули F401 в файловых командах

Отдельных новых команд для модулей нет. Если прошивка собрана с загружаемыми
FOCAL, TinyBASIC или WBMP viewer, существующие файловые команды дополнительно
обслуживают три корневых псевдофайла:

| Команда | Поведение для `.MOD` |
| --- | --- |
| `ls /`, `ls /FOCAL.MOD` | Показывает только действительно установленный контейнер и его размер. |
| `fsget /FOCAL.MOD` | Скачивает контейнер без преобразований. |
| `fsput begin/data/end` | Устанавливает или заменяет контейнер после полной проверки. |
| `rm /FOCAL.MOD` | Стирает выделенный слот модуля. |
| `mv /FOCAL.MOD ...` | Всегда отклоняется: имя и положение фиксированы. |
| `df` | Учитывает установленные модули в числе видимых файлов, но не в узлах C5. |

То же относится к `BASIC.MOD` и `WBMP.MOD`, а также к соответствующим
синонимам команд. Имена сравниваются без учёта регистра, но в листинге всегда
выводятся канонически. Подкаталоги и произвольные имена `.MOD` не принимаются.
...
один файл. В F401-сборке установленные `.MOD` добавляются к списку корня.
`dir` и `fsls` — синонимы `ls`.
...
собственного поддерева или сделать дерево глубже 32 уровней. Имена корневых
`.MOD` фиксированы, поэтому `mv` для них отклоняется.
...
нельзя. В F401-сборке `rm /FOCAL.MOD`, `rm /BASIC.MOD` и `rm /WBMP.MOD`
стирают соответствующий модульный слот. `del` и `fsrm` — синонимы `rm` и также
принимают `-r`.
...
`@MKC:DONE`. Ошибка имеет вид `@MKC:ERROR <code>`; код `PUT_FIRMWARE`
означает, что `.MOD` собран для другой resident-прошивки.
...
размеру. Если сборка включает загружаемые модули, `fsget` и `fsput`
дополнительно обслуживают только корневые `FOCAL.MOD`, `BASIC.MOD` и
`WBMP.MOD`. Первые два контейнера ограничены размером своего 16-КиБ слота,
последний — 4-КиБ слотом.

При `fsput` контейнер сначала целиком принимается в общий 20-КиБ SRAM-overlay,
а после проверки транспортной суммы переносится во временные записи staging
C5. Это нужно потому, что проверка ZX0 использует тот же overlay как выходной
буфер. Прошивка проверяет заголовок, тип слота, привязку к точному resident,
CRC сжатых данных и CRC распакованного образа до стирания старого слота. Затем
контейнер перечитывается из staging и записывается во внешнюю SPI NOR; временные
записи удаляются. Ошибка формата или CRC сохраняет прежний модуль. Потеря
питания уже во время стирания или программирования единственного слота может
оставить модуль отсутствующим или повреждённым — тогда достаточно повторно
скопировать `.MOD` из того же комплекта; C5 и resident-прошивка не страдают.
