...
| `print` | Вывод строки, байтовых escapes и регистров; прежде всего для M61. |
...
Команда не очищает экран и не добавляет перевод строки. Она понимает `\\`,
`\"`, `\r`, `\n`, `\t`, `\b`, `\xHH`, подстановки стека `X/Y/Z/T/X1/X2` и
памяти `R0`...`RF` (также короткие `0`...`F`). `RF` требует расширенного
режима, `\x00` запрещён протоколом USB Screen. В интерактивном терминале
команда тоже доступна, но основной сценарий использования — `.m61`.
