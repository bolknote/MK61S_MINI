...
| `rtc` | Состояние, чтение и установка часов RTC. |
...
| Команда | Назначение |
| --- | --- |
| `save` | Сохранение программы M61 с подтверждением. |
| `load` | Загрузка программы M61. |
| `dir` | Каталог всех поддерживаемых типов файлов. |
| `del` | Немедленное удаление файла. |
...
`RTC UNSET` означает, что пользовательское время еще не задавалось либо была
потеряна резервная область RTC. Это не ошибка команды. Формат строгий, годы
допустимы от 2000 до 2099, календарная дата проверяется с учетом високосных
лет. Прошивка хранит введенное местное время и не пересчитывает часовой пояс.
Полное описание приведено в документе `MK61s-mini-RTC.md`.
...
Хранилище поддерживает типы `M61`, `FOC`, `TBI`, `T1`, `M2` и `FMK`.
Внутреннее имя ограничено 15 символами. Команды с номером слота работают с
историческим представлением программ M61 под именами `0`…`99`.
...
Если расширение не задано, поиск идет по внутреннему имени. Подробности типов и
правил M61 приведены в `MK61s-mini-M61.md`.
...
Принимается номер `0`…`99` или имя M61 с необязательным расширением. Перед
записью терминал всегда ожидает отдельную строку `Y`/`y`; `N`/`n` отменяет
операцию. Пустая программа не сохраняется.
...
Обе команды печатают имя, расширение и размер каждого файла, а затем число
занятых записей из максимально допустимых 128.

### `del` и `fsrm` — удалить файл
...
Без расширения подразумевается M61. Расширения распознаются без учета регистра.
Удаление выполняется немедленно, без запроса подтверждения.
...
Номер лежит в диапазоне `0`…`99`, новое имя содержит от 1 до 15 символов.
Переименование не выполняется, если исходного числового файла нет или новое имя
уже занято.
...
Команда форматирует все внутреннее хранилище, а не только числовые M61-слоты.
Будут удалены файлы всех типов. Требуется отдельное подтверждение `Y`/`y`,
отмена — `N`/`n`.
...
Немедленно удаляет все записи нулевой длины и печатает их количество. Полезно
после оборванного или некорректного импорта через виртуальный USB-диск.
...
терминала автоматически не получает права сценария. В частности, `rtc`,
`history`, команды удаления, форматирование, перезагрузка и DFU в M61 запрещены.
...
`rst` подтверждается на самом устройстве. `del`, `fsrm`, `fsclean`, `snm` и
`dfu` выполняются без терминального запроса. Для опасных команд проверяйте имя и
тип файла до нажатия Enter.

## Что изменилось относительно старой инструкции

- Добавлены `rtc`, `help`, `history`, `ring`, `cmd`, `open`, `if`, `led`,
  `beep`, именованные `save`/`load`, файловые `dir`/`del`, `fsls`, `fsrm`,
  `fsclean` и `vlog`.
- `run` теперь умеет открывать файл, а в M61 — переходить на метку.
- `snm` принимает имя длиной до 15 символов; ошибочная форма `sname` удалена из
  примеров.
