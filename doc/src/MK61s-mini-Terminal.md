...
`TXT`, `STATE.TXT` и `FMK`. Размер одного файла ограничен 1536 байтами,
...
для 512 КиБ доступно 192 узла, для штатной W25Q128 на 16 МиБ — 3939. Файл,
...
`run <path>` является синонимом `open <path>`; `run` без аргумента запускает
...
- Добавлены `rtc`, `help`, `history`, `ring`, `cmd`, `open`, `if`, `led`,
  `beep`, именованные `save`/`load`, дерево C5 и команды `pwd`, `cd`, `ls`,
