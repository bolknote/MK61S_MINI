...
сон только в верхнем idle остановленного калькулятора; MSC, USB Screen,
открытый terminal, звук, клавиша или debounce, CLASSIC, отложенное действие,
вложенный viewer/menu и отсутствие SysTick wake его запрещают. SysTick остаётся
включённым и будит CPU не позднее следующего миллисекундного тика, после чего
обычный scanner проверяет все строки клавиатуры. STOP/STANDBY и изменение
clock tree не используются. `prof start`/`prof reset` очищают также эти
счётчики.
