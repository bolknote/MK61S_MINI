...
5. Выберите `Встроен`, `APP` или `Выключен` для FOCAL, TinyBASIC,
...
WBMP-картинки и CHIP-8 отображаются только на
...
если компонент выключен или встроен. Другие каталоги и пользовательские APP сборщик
платы не трогает.
