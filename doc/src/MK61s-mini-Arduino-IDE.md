...
Shell-путь `tools/build_f401_bundle.sh` запускает отдельные Arduino-сборки с
LTO и host-упаковщик ZX0. Для полностью ручного пути это потребовало бы
...
ARM-компилятор STM32 Core и контейнер `NONE`, но собирает исходники всех четырёх
модулей из отдельного каталога `system_apps/`. Если важен минимальный размер
файлов C5, используйте `tools/build_f401_bundle.sh`: его результат будет ZX0.
Если важна сборка одной кнопкой в Arduino IDE без внешнего toolchain,
используйте эту плату.
