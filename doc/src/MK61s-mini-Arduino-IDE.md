...
APP нельзя записать в C6, пока устройство находится в DFU, поэтому установка
состоит из второго шага:

1. дождитесь запуска новой прошивки;
2. на MK61s откройте `Меню → USB-диск`;
3. на диске `MK61S C6` создайте `/System`, если каталога ещё нет;
...
`tools/mkc.cmd`. При первом переходе на ABI 6 обновите resident и файлы
