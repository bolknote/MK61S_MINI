...
`tools/mkc.cmd`. При первом переходе на ABI 4 обновите resident и файлы
...
Плата `MK61s F401 + APP` вызывает тот же самостоятельный SDK ABI 4, что
...
RAM, ниже защиты стека. Постоянного 20-КиБ резерва нет. Прежние APP ABI 2/3
нужно пересобрать; готовые ABI 4 продолжают работать без изменений.
...
из секции ELF, которая не занимает Flash устройства; копируйте весь `System`. Прежний несжатый режим
Arduino IDE к штатным ABI 4 больше не относится. Написание собственных
...
| `app/firmware mismatch` | Для ABI 2 нужен точный комплект; для ABI 4 проверьте поддержку загрузчика и API прошивки. |
