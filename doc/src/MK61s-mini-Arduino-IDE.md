...
каталога `system_apps/` и упаковывает их в `ZX0` нативным C++17-инструментом.
Если важна сборка одной кнопкой без host-компилятора, используйте плату Arduino
IDE: только её post-build по-прежнему создаёт `NONE`. Legacy-сборщик нужен для
custom manifest APP.
