...
CHIP-8 по умолчанию выключен. WBMP-картинки и CHIP-8 отображаются только на
...
`tools/mkc.cmd`. При первом переходе на ABI 3 обновите resident и файлы
`/System`. Новые APP используют версионированный C API, поэтому последующее
обновление совместимой прошивки не требует их пересборки.
Точная привязка к размеру и CRC `.bin` сохраняется у старого ABI 2.
...
Плата `MK61s F401 + APP` вызывает тот же самостоятельный SDK ABI 3, что
и командные сборщики. Каждый System APP связывается для фиксированного
окна `0x20000000`, без импорта адресов из resident ELF. Упаковщик выбирает
меньший из ZX0 и ARM Thumb BCJ + ZX0. Все штатные образы вместе с BSS
по отдельности укладываются в 20 КиБ; одновременно в окне находится один APP.

Канонический путь `tools/mk61-firmware.cmd` на Windows и macOS/Linux вызывает
`tools/build-gcc.cmd`, собирает исходники всех пяти модулей из отдельного
каталога `system_apps/` с тем же выбором сжатия. Прежний несжатый режим
Arduino IDE к штатным ABI 3 больше не относится. Написание собственных
...
| `app/firmware mismatch` | Для ABI 2 нужен точный комплект; для ABI 3 проверьте поддержку загрузчика и API прошивки. |
