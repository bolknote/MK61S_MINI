...
Меню «Математика» предлагает четыре реализации. `CORE` — самый компактный
штатный выбор F401 с точностью ядра МК-61; `FLOAT` использует аппаратные
float-функции Cortex-M4F и примерно семь значащих цифр; `libm` сохраняет
double-точность, но занимает больше всего Flash. `FLOAT` проверяется в CI на
F401 mini V3/A00. Для тесных Classic V2/UC1609 выбирайте `CORE`.
...
20-КиБ APP-арену. Обычная арифметика не переводится на `float`.
...
- обязательные `System/SETUP.APP`, `System/HELP0.TXT` и `System/HELP1.TXT`;
...
`tools/mkc.cmd`. При первом переходе на ABI 5 обновите resident и файлы
...
Плата `MK61s F401 + APP` вызывает тот же самостоятельный SDK ABI 5, что
...
нужно пересобрать; готовые ABI 5 продолжают работать без изменений.

Канонический путь `tools/mk61-firmware.cmd` на Windows и macOS/Linux вызывает
`tools/build-gcc.cmd`, собирает шесть модулей через общий SDK с тем же выбором сжатия.
