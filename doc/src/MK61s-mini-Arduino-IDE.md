...
APP от другой платформы или экрана. Закройте Tera Term/Serial Monitor, если
они удерживают этот порт. При ошибке IDE покажет неуспешную загрузку, даже
если resident уже прошился; повторно прошивать его для установки APP не надо.
...
| `System APP installation failed` | Resident уже может быть прошит. Проверьте COM-порт и закройте терминал, затем выполните показанную выше команду `mkc.cmd --install-system` без повторного DFU. |
