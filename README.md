...
**Как писать самостоятельные APP на C и C++ (ABI 5):**
...
**Готовые игры, примеры, UI-шрифты, пользовательские APP и CHIP-8 ROM для C5:**
...
отдельно: `CORE`, `CORE + APP FLOAT`, `FLOAT` или `LIBM`. Выбранные
...
на UC1609. `FLOAT` вызывает аппаратные `sinf`/`cosf`/… с примерно семью
значащими цифрами и занимает меньше Flash, чем double-`LIBM`; проверочная
mini V3/A00-сборка F401 оставляет 23 496 байт. Полный `LIBM` остаётся ручным
вариантом для конфигураций, где помещается. Для тесных графических профилей
F401, в частности Classic V2/UC1609, по-прежнему следует выбирать `CORE`.
...
семь значащих цифр. Обычная арифметика языков остаётся `double`.
...
ABI 5. Тот же формат и runtime используются на F411. APP используют таблицы C API и не привязаны к адресам функций или CRC
...
переходе на ABI 5 обновите и прошивку, и `/System`;
...
обязательные `System/SETUP.APP`, `System/HELP0.TXT`, `System/HELP1.TXT`. Канонический
...
Пункт второго шага устанавливает SETUP и справку, затем синхронизирует
...
`/System/CHIP8.APP`, `/System/SETUP.APP`. Минимальная точка входа, правила
...
наличии `pwsh`, затем делегирует общему Python-сборщику ABI 5. ARM-код
...
FOCAL, TinyBASIC, просмотрщики, CHIP-8 и SETUP в `/System`. Обычный APPLICATION
отличается только тем, что выбирается по inode, а System APP — по каноническому
имени. Декодирование, релокации, API, MPU и кэш у них общие. Уже отформатированный C5
переформатировать не нужно:
