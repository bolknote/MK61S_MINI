...
значения явно передаются сборке как `-D`ключи. `Markdown + WBMP` обслуживает
и `T2/.md`, и `I1/.wbmp`; отдельный `WBMP viewer` выбирается только вместо
Markdown. Markdown включён, а USB-экран и CHIP-8 по умолчанию выключены. При выключенном
...
Отображение WBMP и CHIP-8 требует графический UC1609 либо скомпилированный
USB-экран. На LCD1602 A00/A02 картинку можно открыть только во время активной
...
`MARKDOWN.APP` на графическом экране обрабатывает оба типа `T2` и `I1`,
поэтому при `MK61_ENABLE_MARKDOWN_VIEWER=1` отдельный `WBMP.APP` не создаётся.
Он появляется только при Markdown=0 и WBMP=1. Общий
...
`WBMP.APP` и `MARKDOWN.APP` при этом взаимоисключающие.
...
Она последовательно выполняет строгий host-набор, все десять F411-вариантов
...
в документе. Отдельный `WBMP.APP` собирается только при выключенном Markdown.
