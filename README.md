...
Пункт второго шага синхронизирует только три канонических System APP:
...
`/System/BASIC.APP` и `/System/WBMP.APP`. Минимальная точка входа, правила
...
Для ручной сборки штатных `FOCAL.APP`, `BASIC.APP` и `WBMP.APP` есть отдельная
плата `MK61s F401 + APP`. Один раз установите её из корня проекта:
...
4. оставьте `FOCAL`, `TinyBASIC` и `WBMP viewer` в положении `APP` либо
   выключите ненужные компоненты;
