...
USB-диск либо через правую панель `tools/mkc.cmd`. В MKC установленные модули
видны в корне вместе с программами; F3 показывает контейнер в hex, F5 позволяет
скачать или заменить его, F8 — удалить. Имена фиксированы, поэтому перемещать
или переименовывать `FOCAL.MOD`, `BASIC.MOD`, `WBMP.MOD` нельзя.

Это обычные заменяемые файлы без установленных атрибутов: новую согласованную
версию можно скопировать поверх старой, а ненужный модуль удалить. Перед
установкой прошивка проверяет тип, размер, CRC контейнера и совпадение
resident-прошивки; повреждённый или чужой модуль рабочую копию не заменяет.
