...
**Как писать самостоятельные APP на C и C++ (ABI 4):**
...
Штатный профиль оптимизации — `Smallest (-Os)`: он даёт максимальный запас
Flash без замедления измеренных горячих путей МК-61. Если в обычном меню платы
Arduino вручную выбрать `-O2` или `-O3`, прошивка применяет смешанную политику:
парсеры, меню и диагностические форматтеры остаются компактными (`-Os`), а
ядро эмуляции сохраняет выбранную оптимизацию скорости. Это не позволяет GCC
размножить холодный код до переполнения 512 КиБ. CI отдельно собирает самый
крупный Classic V3 с `-O3` и требует не менее 64 КиБ остатка Flash.
...
ABI 4. APP используют таблицы C API и не привязаны к адресам функций или CRC
точной прошивки. Внутренние указатели исправляются по таблице релокаций;
APP арендует только свой размер в общем 20-КиБ пуле. При первом переходе на ABI 4 обновите и прошивку, и `/System`;
...
квалификационному профилю WS0010 100x16. Загрузчик системных компонентов F401
при этом остаётся включённым, но обычные пользовательские `.APP` запускаются
только с `MK61_ENABLE_USER_APPS=1`.
...
Как и GCC-путь, он создаёт самостоятельные APP ABI 4 с ZX0 или BCJ + ZX0.
...
отключается ключом `-Lto 0`, пользовательские APP разрешаются ключом
`-UserApps 1`, а `-Clean` пересоздаёт рабочий каталог профиля.
...
наличии `pwsh`. По умолчанию он вызывает самостоятельный SDK ABI 4;
...
задаёт компилятор; в ABI 4 адреса resident в APP не импортируются.
...
пользовательские APP ABI 2; обе возможности нужно включить явно:

```bash
MK61_ENABLE_USER_APPS=1 MK61_ENABLE_PORTABLE_APPS=0 \
...
оставлен для нестандартной ручной сборки. Это не разрешает запуск произвольных
APP: `MK61_ENABLE_USER_APPS` по умолчанию равен `0`. При его включении на F411
конфигуратор добавляет загрузчик и переносимую SRAM-разметку, но оставляет
FOCAL, TinyBASIC и просмотрщики во внутренней Flash. Уже отформатированный C5
