...
сохранённых настроек у Bash- и PowerShell-версий одинаковый. На Windows
`dfu-util.exe` дополнительно ищется в `PATH`, Scoop, Chocolatey и
`%LOCALAPPDATA%\Arduino*\packages\STMicroelectronics\tools\STM32Tools\*\win`.
Для Unicode-псевдографики и палитры рекомендуется Windows Terminal.
...
При старте быстро ищется только исполняемый `dfu-util` — USB в этот момент не
сканируется. Проверяются сохранённый путь, `PATH`, Homebrew/MacPorts,
...
ставит.
DFU-загрузка выполняется утилитой `dfu-util`, которая входит в пакет STM32 Core,
так что STM32CubeProgrammer не требуется.
