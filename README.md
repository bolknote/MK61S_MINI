...
(`-Os`). F401 сохраняет прежние два локальных `-O3`-цикла и компактный
битовый декодер. Host-тест запускает оба пути из одного снимка и после каждого
шага сравнивает полные 672 байта состояния, а release gate проверяет выбранный
путь в ELF.
