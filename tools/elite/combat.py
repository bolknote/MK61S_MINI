...
def add_combat(a):
    ...
    m.get(25,6).n(1000000).op('/','int').n(12).op('*').n(20).op('+').ld(7).op('-').st('D')
    m.ld(2).st('C').visit(24,'weapon').ld('C').st(3)
    m.ld(7).n(19).op('-').jneg('range_ready')
    m.set(4,0).ld(2).n(1).op('-').jnz('range_ready').set(3,0)
    m.label('range_ready').ld(1).n(4).op('-').jnz('shot_ready')
    m.ld(3).n(2).op('/','int').st(3)
    m.label('shot_ready').ld(3).st('C').ld(5).st('D').ld(8).st('E').visit(28,'drone_tick')
    m.ld('D').st(6).ld(7).n(19).op('-').jge('incoming_ready')
    m.ld(4).ld('C').op('+').st(4)
    ...
    # PILOT callback: action RC, laser damage RD -> shot RC.
    m.label('weapon').ld(7).n(10).op('-').call('max0').st(7)
    m.ld('C').n(1).op('-').jz('laser')
    m.ld('C').n(2).op('-').jz('missile')
    m.ld('C').n(3).op('-').jnz('no_shot')
    m.add(5,18).n(60).op('-').jneg('no_shot').set(5,60)
    m.label('no_shot').set('C',0).op('ret')
    m.label('laser').ld(7).n(70).op('-').jge('no_shot').add(7,28).ld('D').st('C').op('ret')
    m.label('missile').add(7,10).set('C',45).op('ret')
    ...
    # carrier's pre-hit attack RE, launch flag RF. All shots are simultaneous.
    m.label('enemy_tick').ld('C').st(3).n(4).op('-').jz('motion_ready')
    m.ld('C').n(2).op('-').n(2).op('*').ld(2).op('+').call('max0').st(2)
    ...
    m.ld('A').n(9).op('-').jz('range_view')
    m.ld('A').n(16).op('-').jz('target_view')
