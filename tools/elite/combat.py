...
def add_combat(a):
    ...
    m.label('fight_command').ld(0).n(10).op('/','int').st(1).n(1).op('-').jneg('bad_action')
    m.ld(1).n(5).op('-').jge('bad_action')
    m.ld(0).call('mod10').st(2).n(1).op('-').jneg('bad_action')
    ...
    m.label('range_ready').ld(3).st('C').ld(5).st('D').ld(8).st('E').visit(28,'drone_tick')
    ...
    m.add(5,18).n(60).op('-').jneg('no_shot').set(5,60).jump('no_shot')
    ...
    m.label('drone_tick').set('B',0).set('F',0)
    m.label('count_drone').raw(0xDB).jz('next_drone').add('F',1)
    m.label('next_drone').add('B',1).n(5).op('-').jneg('count_drone')
    m.ld('F').st(6).ld('E').jz('drones_hit').ld(5).n(5).op('-').jge('drones_hit')
