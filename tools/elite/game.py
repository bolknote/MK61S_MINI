...
def create_game():
    ...
    m.label('stat_prefix').ptr('E','stat_masks').ld('A').n(1).op('-').n(4).op('*').ld('E').op('+').st('E').raw(0x1F,0xAE).st('D')
    m.ld('A').n(1).op('-').jz('draw_number').n(5).op('-').jz('draw_number')
    m.set('E',20).ld('A').n(3).op('-').jnz('draw_gauge').set('E',12).jump('draw_gauge')
    m.label('show_hold').visit(25,'sum_hold').set('D',GLYPHS['U']).set('E',4).jump('draw_gauge')
