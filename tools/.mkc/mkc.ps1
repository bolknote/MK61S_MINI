...
function Save-Config {
    ...
    $text = "# Generated by tools/mkc.cmd.`nLOCAL_PATH=$($script:LocalPath)`nPORT=$($script:Port)`nARDUINO_CLI=$($script:ArduinoCli)`n"
...
function Update-Layout {
    ...
    # Some pseudo terminals report 0×0 until the first resize event.  A sane
    # fallback keeps startup deterministic; an actual small console is still
    # rejected below.
