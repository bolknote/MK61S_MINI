...
Обычному запускаемому APP достаточно `name` и `source`. APP, который должен
открывать отдельный тип C5, дополнительно указывает двухбайтовую регистрацию,
например `magic C2`, и обрабатывает команду `FILE_OPEN`. Директива не создаёт
сам тип или расширение в resident.
